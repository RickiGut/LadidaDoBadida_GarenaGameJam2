...
namespace RomDev
{
	...
	public class SceneSettings : MonoBehaviour, iActionListAssetReferencer
	{
		...
		#region Variables
		/** Instantiated Prefab data in a scene */
		public Dictionary<GameObject, int> instantiatedDatas = new();

		/** Destroyed original GameObject in a scene */
        public HashSet<int> destroyedSceneObjs = new();
		...
		public void AddInstantiatedObject(GameObject _obj, int prefabID)
        {
            instantiatedDatas.Add(_obj, prefabID);
        }
        public bool TryDestroySceneObject(GameObject _obj)
        {
            if( ! _obj.TryGetComponent(out BasicObjectDataSaver basicDataSaver))
            {
                return false;
            }
            if( ! KickStarter.stateHandler.ConstantIDManager.IsContains(basicDataSaver) )
            {
                return false;
            }
            destroyedSceneObjs.Add(basicDataSaver.constantID);
            return true;
        }
        public bool TryDeleteCloneObject(GameObject _obj)
        {
            if( ! instantiatedDatas.ContainsKey(_obj))
            {
                return false;
            }
            instantiatedDatas.Remove(_obj);
            return true;
        }
