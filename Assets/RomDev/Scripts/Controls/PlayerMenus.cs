...
namespace RomDev
{
	...
	public class PlayerMenus : MonoBehaviour
	{
		...
		public void RebuildMenus (MenuManager menuManager = null)
		{
			...
			foreach (RomDev.Menu menu in menus)
			{
				menu.Recalculate ();
			}
		...
		{
			foreach (RomDev.Menu menu in KickStarter.playerMenus.menus)
			{
		...
		{
			foreach (RomDev.Menu _menu in GetMenus (true))
			{
		...
		protected void OnMouseOverMenu (RomDev.Menu menu, MenuElement element, int slot)
		{
			if (element != null)
			{
		...
		{
			foreach (RomDev.Menu _menu in menus)
			{
		 ...
		 */
		public void DrawMenu (RomDev.Menu menu, int languageNumber = 0)
		{
			Color tempColor = GUI.color;
		 ...
		 */
		public void UpdateMenuPosition (RomDev.Menu menu, Vector2 invertedMouse, bool force = false)
		{
		 ...
		 */
		public void UpdateMenu (RomDev.Menu menu, int languageNumber = 0, bool justPosition = false, bool updateElements = true)
		{
			...
			switch (menu.appearType)
			{
				...
				case AppearType.OnContainer:
					if (KickStarter.playerInput.activeContainer && !menu.isLocked && (KickStarter.stateHandler.IsInGameplay () || (KickStarter.stateHandler.gameState == RomDev.GameState.Paused && menu.IsBlocking ())))
					{
					...
					else
					{
						if (menu.IsEnabled () && (KickStarter.stateHandler.IsInGameplay () || menu.pauseWhenEnabled || (KickStarter.stateHandler.IsPaused () && InvInstance.IsValid (menu.TargetInvInstance) && menu.GetGameStateWhenTurnedOn () == GameState.Paused)))
						{
							...
							else if (!menu.IsPointInside (invertedMouse) && !menu.ignoreMouseClicks && KickStarter.settingsManager.interactionMethod == AC_InteractionMethod.ChooseHotspotThenInteraction && KickStarter.settingsManager.cancelInteractions == CancelInteractions.CursorLeavesMenu && KickStarter.settingsManager.SelectInteractionMethod () == SelectInteractions.ClickingMenu && !menu.IsFadingIn ())
							{
								...
								KickStarter.settingsManager.interactionMethod == AC_InteractionMethod.ChooseHotspotThenInteraction && KickStarter.settingsManager.selectInteractions == RomDev.SelectInteractions.CyclingMenuAndClickingHotspot)
							{
								menu.TurnOff (true);
							}
		...
		protected void UpdateElements (RomDev.Menu menu, int languageNumber, bool justDisplay = false)
		{
			if (!menu.HasTransition () && menu.IsFading ())
			{
				// Stop until no longer "fading" so that it appears in right place
		...
		protected bool SlotIsInteractive (RomDev.Menu menu, int elementIndex, int slotIndex, GameState gameState)
		{
		...
		protected bool CheckClicks (RomDev.Menu menu)
		{
			if (!menu.HasTransition () && menu.IsFading ())
			{
				// Stop until no longer "fading" so that it appears in right place
		 ...
		 */
		public void CheckCrossfade (RomDev.Menu _menu)
		{
		...
		protected void CheckClick (RomDev.Menu _menu, MenuElement _element, int _slot, MouseState _mouseState)
		{
		...
		protected bool CheckContinuousClick (RomDev.Menu _menu, MenuElement _element, int _slot, MouseState _mouseState)
		{
		 ...
		 */
		public void CrossFade (RomDev.Menu _menuTo)
		{
			if (_menuTo.isLocked)
			{
		...
		{
			Menu[] allMenus = GetMenus (true).ToArray ();
			foreach (RomDev.Menu _menu in allMenus)
			{
		...
		public static void CreateRecipe ()
		{
			...
			if (allMenus != null)
			{
				foreach (RomDev.Menu menu in allMenus)
				{
					foreach (MenuElement menuElement in menu.elements)
					{
		...
		public static void SimulateClick (string menuName, string menuElementName, int slot = 1)
		{
			if (KickStarter.playerMenus)
			{
				RomDev.Menu menu = PlayerMenus.GetMenuWithName (menuName);
		...
		public void ForceOffSubtitles (SpeechMenuLimit speechMenuLimit = SpeechMenuLimit.All)
		{
			...
			foreach (RomDev.Menu menu in dupSpeechMenus)
			{
				ForceOffSubtitles (menu, speechMenuLimit);
			}

			foreach (RomDev.Menu menu in customMenus)
			{
				ForceOffSubtitles (menu, speechMenuLimit);
			}
		}
