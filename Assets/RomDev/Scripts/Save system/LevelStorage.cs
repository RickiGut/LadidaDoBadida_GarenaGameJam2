...
namespace RomDev
{
	...
	public class LevelStorage : MonoBehaviour
	{
		...
		Dictionary<int, SaveBasicData> tempBasicDataDict = new();
		...
		public  GameObject InstantiateObject(GameObject sourcePrefab, string targetScene = "")
        {
            GameObject theObj = Instantiate(sourcePrefab);
			int prefabId = GetPrefabId(sourcePrefab);
			if(prefabId == -1)
			{
				return theObj;
			}
			...
			{
				KickStarter.sceneSettings.AddInstantiatedObject(theObj, prefabId);
			}else
			{
				SubScene subScene = KickStarter.sceneChanger.GetSubScene(targetScene);
				if(subScene != null)
				{
					UnityEngine.SceneManagement.SceneManager.MoveGameObjectToScene (theObj, UnityEngine.SceneManagement.SceneManager.GetSceneByBuildIndex(subScene.SceneIndex));
					subScene.SceneSettings.AddInstantiatedObject(theObj, prefabId);
				}
			} 
            return theObj;
        }
        public  GameObject InstantiateObjectWithParent(GameObject sourcePrefab, Transform targetParent)
        {
            GameObject theObj =  Instantiate(sourcePrefab, targetParent);
			Scene targetScene = theObj.scene;
            int prefabId = GetPrefabId(sourcePrefab);
			if(prefabId == -1)
			{
				return theObj;
			...
			{
				SubScene subScene = KickStarter.sceneChanger.GetSubScene(targetScene.name);
				if(subScene != null)
				{
					subScene.SceneSettings.AddInstantiatedObject(theObj, prefabId);
				}
		...
		public  bool DeleteObject(GameObject targetObj)
        {
            if(targetObj != null)
            {
				Scene sceneSource = targetObj.scene;
                BasicObjectDataSaver basicDataSaver = targetObj.GetComponent<BasicObjectDataSaver>();
                if(basicDataSaver != null)
                {
                    if( ! targetObj.name.Contains("(Clone)"))
                    {
                        if(sceneSource == SceneChanger.CurrentScene)
						{
							KickStarter.sceneSettings.TryDestroySceneObject(targetObj);
						}else
						{
							SubScene subScene = KickStarter.sceneChanger.GetSubScene(sceneSource.name);
							if(subScene != null)
							{
								subScene.SceneSettings.TryDestroySceneObject(targetObj);
							}
						}
                    }else
                    {
						if(sceneSource == SceneChanger.CurrentScene)
						{
							KickStarter.sceneSettings.TryDeleteCloneObject(targetObj);
						}else
						{
							SubScene subScene = KickStarter.sceneChanger.GetSubScene(sceneSource.name);
							if(subScene != null)
							{
								subScene.SceneSettings.TryDeleteCloneObject(targetObj);
							}
						}
		...
		private int GetPrefabId(GameObject sourcePrefab)
        {
            if( ! KickStarter.settingsManager.PrefabDictObj.TryGetValue(sourcePrefab, out int prefabID))
            {
                Debug.LogError($"There's no prefab with name : {sourcePrefab.name} ,inside asset");
                return -1;
            }
			return prefabID;
        }
		...
		public IEnumerator ReturnCurrentLevelData ()
		{
			SingleLevelData levelData = GetLevelData ();

			if (levelData != null)
			{
				var loadSceneDataCoroutine = LoadSceneData (levelData, null);
		...
		public IEnumerator LoadPlayerData (Player player, PlayerData playerData)
		{
			Remember[] playerSaveScripts = player.gameObject.GetComponentsInChildren<Remember> ();
				...
				foreach (ScriptData _scriptData in playerData.playerScriptData)
				{
					if (!string.IsNullOrEmpty (_scriptData.data))
					{
						foreach (Remember playerSaveScript in playerSaveScripts)
						{
							...
							{
								var loadPlayerDataCoroutine = playerSaveScript.LoadDataCo (_scriptData.data);
		...
		public IEnumerator LoadPersistentData (MainData mainData)
		{
			HashSet<Remember> persistentSaveScripts = KickStarter.stateHandler.ConstantIDManager.GetPersistentButNotPlayerComponents <Remember>();
				...
				foreach (ScriptData _scriptData in mainData.persistentScriptData)
				{
					if (!string.IsNullOrEmpty (_scriptData.data))
					{
						foreach (Remember remember in persistentSaveScripts)
						{
							...
							{
								var loadRememberDataCoroutine = remember.LoadDataCo (_scriptData.data);
								while (loadRememberDataCoroutine.MoveNext ())
								{
									yield return loadRememberDataCoroutine.Current;
								}
							}
						}
					}
		...
		private IEnumerator LoadSceneData (SingleLevelData levelData, SubScene subScene = null)
		{
			...
			while (unloadScriptDataCoroutine.MoveNext ())
			{
				yield return unloadScriptDataCoroutine.Current;
			}
			
			if(levelData.allBasicData != null)
			{
				tempBasicDataDict.Clear();

				IEnumerator unloadFirstFinalDataRoutine = UnloadFirstFinalData(levelData.allBasicData, scene);
				while (unloadFirstFinalDataRoutine.MoveNext ())
				{
					yield return unloadFirstFinalDataRoutine.Current;
				}

				IEnumerator unloadSecondFinalDataRoutine = UnloadSecondFinalData(levelData.allBasicData, scene);
				while (unloadSecondFinalDataRoutine.MoveNext ())
				{
					yield return unloadSecondFinalDataRoutine.Current;
				}
				tempBasicDataDict.Clear();
			}
		}
		...
		private void SaveSceneData (SubScene subScene = null)
		{
			...
			FinalSaveData thisFinalSaveData = PopulateBasicSaverData(scene);
			...
			thisLevelData.allBasicData = thisFinalSaveData;
		...
		private IEnumerator UnloadScriptData (List<ScriptData> allScriptData, Scene scene)
		{
			...
			foreach (ScriptData _scriptData in allScriptData)
			{
				if (!string.IsNullOrEmpty (_scriptData.data))
				{
					...
					{
						if (saveObject == null || !saveObject.isActiveAndEnabled) continue;
		...
		private IEnumerator UnloadFirstFinalData (FinalSaveData finalSaveData, Scene scene)
		{
			// HashSet<BasicObjectDataSaver> basicDatas = ConstantID.GetComponents <BasicObjectDataSaver> (scene);
			foreach (InstantPrefabData instantiatedData in finalSaveData.instantiatedDatas)
            {
                if( ! KickStarter.settingsManager.PrefabDict.TryGetValue(instantiatedData.prefabID, out GameObject sourcePrefab))
                {
                    continue;
                }
                BasicObjectDataSaver objPersistent = InstantiateObject(sourcePrefab, scene.name).GetComponent<BasicObjectDataSaver>();
				if( ! KickStarter.stateHandler.ConstantIDManager.OverwriteBasicSaverID(objPersistent, instantiatedData.basicSaverID))
				{
					objPersistent.constantID = instantiatedData.basicSaverID;
				}
            }
            //
            foreach (int destroyedObjID in finalSaveData.destroyedObjIDs)
            {
				BasicObjectDataSaver destroyedBasicData = ConstantID.GetComponent<BasicObjectDataSaver> (destroyedObjID);
                if( destroyedBasicData == null)
                {
                    continue;
                }
                DeleteObject(destroyedBasicData.gameObject);
            }

			foreach (KeyValuePair<int, SaveBasicDataL2> basicSaverPair in finalSaveData.basicDataL2Dict.GetAsDictionary())
            {
                SaveBasicData saveBasicData = new();
                tempBasicDataDict.Add(basicSaverPair.Key, saveBasicData);
                saveBasicData.saveDict = basicSaverPair.Value.saveDict;
                foreach (KeyValuePair<int, ComponentSaveDataL2> compDataPair in basicSaverPair.Value.compDataDict.GetAsDictionary())
                {
                    ComponentSaveData _compData = new();
                    _compData.compFullName = compDataPair.Value.compFullName;
                    if(KickStarter.settingsManager.SODict.TryGetValue(compDataPair.Value.saveReqID, out ScriptableObject resultSO))
                    {
                        SaveRequirement saveReq = resultSO as SaveRequirement;
                        _compData.saveRequirement = saveReq;
                    }
                    saveBasicData.compDataDict.AddData(compDataPair.Key, _compData);
                }
				BasicObjectDataSaver basicDataSaver = ConstantID.GetComponent<BasicObjectDataSaver> (basicSaverPair.Key);
                if( basicDataSaver == null)
                {
                    continue;
                }
                var loadDataCoroutine = basicDataSaver.LoadComponents(saveBasicData);
				while (loadDataCoroutine.MoveNext ())
				{
					yield return loadDataCoroutine.Current;
				}
		...
		private IEnumerator UnloadSecondFinalData (FinalSaveData finalSaveData, Scene scene)
		{
			foreach (KeyValuePair<int, SaveBasicDataL2> basicSaverPair in finalSaveData.basicDataL2Dict.GetAsDictionary())
            {
                SaveBasicData _saveBasicData = tempBasicDataDict[basicSaverPair.Key];
                foreach (KeyValuePair<int, ComponentSaveDataL2> compDataPair in basicSaverPair.Value.compDataDict.GetAsDictionary())
                {
                    if(_saveBasicData.compDataDict.GetData(compDataPair.Key).saveRequirement.saveData)
                    {
                        SaveDataDict saveDataDict = new();
                        if(_saveBasicData.compDataDict.GetData(compDataPair.Key).saveRequirement.saveSaveDict)
                        {
                            _saveBasicData.compDataDict.GetData(compDataPair.Key).saveDataDict = saveDataDict;
                            
                            DictionaryBridgeL1<string, string> saveDict = JsonUtility.FromJson<DictionaryBridgeL1<string, string>>(compDataPair.Value.saveDataDict.GetData(SaveDataL2.SaveDict));
                            saveDataDict.saveDict = saveDict;
                           
                        }
                        if(_saveBasicData.compDataDict.GetData(compDataPair.Key).saveRequirement.saveCompRefs)
                        {
                            DictionaryBridgeL1<string, ComponentSaveDataV2> compRefs = JsonUtility.FromJson<DictionaryBridgeL1<string, ComponentSaveDataV2>>(compDataPair.Value.saveDataDict.GetData(SaveDataL2.CompRefs));
                            foreach (KeyValuePair<string,ComponentSaveDataV2> compRef in compRefs.GetAsDictionary())
                            {
                                Component targetComp = null;
								BasicObjectDataSaver basicDataSaver = ConstantID.GetComponent<BasicObjectDataSaver>(compRef.Value.basicSaverId);
                                if(basicDataSaver == null)
                                {
                                    continue;
                                }
                                if(basicDataSaver.constantID == compRef.Value.compID)
                                {
                                    targetComp = basicDataSaver;
                                }else if(BasicObjectDataSaver.TryGetComponentFromBasicServer<Component>(basicDataSaver, compRef.Value.compID, out Component resultComp))
                                {
                                ...
                                saveDataDict.compRefs.AddData(compRef.Key, targetComp);
                            }
                        }
                    }
                }
				BasicObjectDataSaver basicDataSaverFinal = ConstantID.GetComponent<BasicObjectDataSaver>(basicSaverPair.Key);
                if( basicDataSaverFinal == null)
                {
                    continue;
                }
                var loadDataCoroutine = basicDataSaverFinal.LoadEveryComponentData(_saveBasicData);
				while (loadDataCoroutine.MoveNext ())
				{
					yield return loadDataCoroutine.Current;
				}
		...
		private FinalSaveData PopulateBasicSaverData (Scene scene)
		{
			FinalSaveData finalSaveData = new FinalSaveData();
			HashSet<BasicObjectDataSaver> basicDatas = ConstantID.GetComponents <BasicObjectDataSaver> (scene);
			SceneSettings sceneSettings;
			if(scene == SceneChanger.CurrentScene)
			{
				sceneSettings = KickStarter.sceneSettings;
			}else
			{
				SubScene subscene = KickStarter.sceneChanger.GetSubScene (scene.name);
				sceneSettings = subscene.SceneSettings;
			}

			//
			DictionaryBridgeL1<int, SaveBasicData> basicDataDict = new();
            foreach (BasicObjectDataSaver _basicData in basicDatas)
            {
                int basicSaverID = _basicData.constantID;
                basicDataDict.AddData(basicSaverID, _basicData.SaveBasicData());
            }
            //
            foreach (KeyValuePair<int, SaveBasicData> basicSaverPair in basicDataDict.GetAsDictionary())
            {
                SaveBasicDataL2 saveBasicDataL2 = new();
                saveBasicDataL2.saveDict = basicSaverPair.Value.saveDict;
                foreach (KeyValuePair<int, ComponentSaveData> compDataPair in basicSaverPair.Value.compDataDict.GetAsDictionary())
                {
                    ComponentSaveDataL2 _compDataL2 = new();
                    _compDataL2.compFullName = compDataPair.Value.compFullName;
					int saveReqID = KickStarter.settingsManager.GetIdOfScriptableObject(compDataPair.Value.saveRequirement);
                    if(saveReqID != -1)
                    {
                        _compDataL2.saveReqID = saveReqID;
                    }else
					{
						Debug.LogWarning($"Can't found id for scriptable object {compDataPair.Value.saveRequirement.name}");
					}
                    if(compDataPair.Value.saveRequirement.saveData)
                    {
                        // SaveDataL2 saveDataL2 = new();
                        if(compDataPair.Value.saveRequirement.saveSaveDict)
                        {
                            DictionaryBridgeL1<string, string> saveDict = new();
                            saveDict = compDataPair.Value.saveDataDict.saveDict;
                            _compDataL2.saveDataDict.AddData(SaveDataL2.SaveDict, JsonUtility.ToJson(saveDict));
                        }
                        if(compDataPair.Value.saveRequirement.saveCompRefs)
                        {
                            DictionaryBridgeL1<string, ComponentSaveDataV2> compRefs = new();
                            foreach (KeyValuePair<string, Component> compRefPair in compDataPair.Value.saveDataDict.compRefs.GetAsDictionary())
                            {
                                ComponentSaveDataV2 compSaveDataV2 = new();
                                if( ! compRefPair.Value.TryGetComponent(out BasicObjectDataSaver _basicSaver))
                                {
                                    continue;
                                }
                                int targetCompID = 0;
                                if(compRefPair.Value == _basicSaver)
                                {
                                    targetCompID = _basicSaver.constantID;
                                }else if(_basicSaver.TryGetComponentID(compRefPair.Value, out int resultCompID))
                                {
                                    targetCompID = resultCompID;
                                }else
                                {
                                    continue;
                                }
                                compSaveDataV2.basicSaverId = _basicSaver.constantID;
                                compSaveDataV2.compID = targetCompID;
                                compRefs.AddData(compRefPair.Key, compSaveDataV2);
                            }
                            _compDataL2.saveDataDict.AddData(SaveDataL2.CompRefs, JsonUtility.ToJson(compRefs));
                        }
                        // _compDataL2.saveDataL2 = saveDataL2;
                    }
                    saveBasicDataL2.compDataDict.AddData(compDataPair.Key, _compDataL2);
                }
                finalSaveData.basicDataL2Dict.AddData(basicSaverPair.Key, saveBasicDataL2);
            }
            //
            List<InstantPrefabData> instantPrefabDatas = new();
            foreach (KeyValuePair<GameObject, int> instantiatedPair in sceneSettings.instantiatedDatas)
            {
                InstantPrefabData instantPrefabData = new();
                instantPrefabData.prefabID = instantiatedPair.Value;
                GameObject _inspectedObj = instantiatedPair.Key;
                instantPrefabData.basicSaverID = _inspectedObj.GetComponent<BasicObjectDataSaver>().constantID;
                instantPrefabDatas.Add(instantPrefabData);
            }
            finalSaveData.destroyedObjIDs = sceneSettings.destroyedSceneObjs.ToArray<int>();
            finalSaveData.instantiatedDatas = instantPrefabDatas.ToArray();
			//
	...
	public class SingleLevelData
	{
		...
		/** A List of all data recorded by the scene's BasicObjectdataSaver scripts */
		public FinalSaveData allBasicData;
		...
		public void RemoveDataForID (int id)
		{
			...
			if(allBasicData.basicDataL2Dict.ContainsKey(id))
			{
				allBasicData.basicDataL2Dict.RemoveData(id);
			}
		}
	...
	[System.Serializable]
    public class InstantPrefabData
    {
        public int prefabID;
        public int basicSaverID;
    }
    [System.Serializable]
    public class FinalSaveData
    {
        public InstantPrefabData[] instantiatedDatas;
        public int[] destroyedObjIDs;
        public DictionaryBridgeL1<int, SaveBasicDataL2> basicDataL2Dict = new();
    }
