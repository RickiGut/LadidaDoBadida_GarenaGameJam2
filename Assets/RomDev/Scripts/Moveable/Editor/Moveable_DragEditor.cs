...
namespace RomDev
{
	...
	public class Moveable_DragEditor : DragBaseEditor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			_target.fireOnDeltaMove = CustomGUILayout.Toggle ("Fire delta track?", _target.fireOnDeltaMove, string.Empty, "Fire delta track valeu ?");
			...
			if (_target.dragMode == DragMode.LockToTrack)
			{
				...
				{
					_target.simulatedMass = CustomGUILayout.Slider ("Simulated mass:", _target.simulatedMass, 0f, 20f, string.Empty, "The object's simulated mass, if not using a Rigidbody");
				}
				else
				{
			...
			{
				_target.interactionStartMove = (Interaction) CustomGUILayout.AutoCreateField ("Interaction on Start move:", _target.interactionStartMove, OnAutoCreateInteractionOnStartMove, "", "The Interaction to run whenever the object is Start Moved by the player");
				if (_target.dragMode == DragMode.LockToTrack)
				{
		...
		private Interaction OnAutoCreateInteractionOnStartMove ()
		{
			Undo.RecordObject (_target, "Create Interaction");
			Interaction newInteraction = SceneManager.AddPrefab ("Logic", "Interaction", true, false, true).GetComponent<Interaction> ();
			newInteraction.gameObject.name = AdvGame.UniqueName (_target.gameObject.name + ": StartMove");
			return newInteraction;
		}
