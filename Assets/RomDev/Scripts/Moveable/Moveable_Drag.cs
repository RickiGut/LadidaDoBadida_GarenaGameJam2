...
namespace RomDev
{
	...
	public class Moveable_Drag : DragBase, iActionListAssetReferencer
	{
		...
		#region Variables
		public bool fireOnDeltaMove;
		public System.Action<float> onDeltaMove;
		public System.Action<float> onStartMove;
		public System.Action<float> onEndMove;
		...
		public Interaction interactionStartMove = null;
		...
		public override void UpdateMovement ()
		{
			...
			float deltaTrack = 0f;
			if ( (moveSoundClip && moveSound) || fireOnDeltaMove)
			{
				deltaTrack = trackValue - lastFrameTrackValue;
			}
			if (moveSoundClip && moveSound)
			{
				if (dragMode == DragMode.LockToTrack && track)
				{
					// Debug.Log(track.GetMoveSoundIntensity (deltaTrack));
					PlayMoveSound (track.GetMoveSoundIntensity (deltaTrack));
				}
				else if (_rigidbody)
				{
			...
			if(fireOnDeltaMove && firstDrag)
			{
				onDeltaMove?.Invoke(deltaTrack);
			}
		...
		public override void Grab (Vector3 grabPosition)
		{
			//
			firstDrag= true;
			interactionStartMove?.Interact();
			onStartMove?.Invoke(trackValue);
			//
			isHeld = true;
		...
		protected void RunInteraction (bool onMove)
		{
			...
			// if( ! onMove) onEndMove?.Invoke(trackValue);
			switch (actionListSource)
			{
				case ActionListSource.InScene:
