		...
		public void ShowGUI (bool isAssetFile)
		{
			...
			if (actionListAsset)
			{
				if (ParameterReferences.Length > 0)
				{
					...
					{
						ParameterIDs[i] = RomDev.Action.ChooseParameterGUI (ParameterReferences[i].Label, actionListAsset.DefaultParameters, ParameterIDs[i], ParameterReferences[i].Type, -1, string.Empty, true);
					}
				}
			}
		}
