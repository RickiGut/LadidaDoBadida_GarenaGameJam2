...
namespace RomDev
{
	...
	public class DetectHotspotsEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			_target.gatherMode = (DetectHotspots.HotspotGatherMode) EditorGUILayout.EnumPopup ("Gather mode:", _target.gatherMode);
			if (_target.gatherMode == DetectHotspots.HotspotGatherMode.CustomScript)
			{
				EditorGUILayout.HelpBox ("Call the component's AddHotspot and RemoveHotspot to modify the collection of detected Hotspots.", MessageType.Info);
			}
			else if (_target.gatherMode == DetectHotspots.HotspotGatherMode.TriggerDetection)
			{
