...
namespace RomDev
{
	...
	public class NavigationEngine_AStar2D : NavigationEngine
	{
		...
		private readonly RomDev.AStar2D.Pathfinding pathfinding = new RomDev.AStar2D.Pathfinding ();
		...
		private void AddCharHoles (Grid2D grid, RomDev.Char charToExclude, NavigationMesh navigationMesh)
		{
			...
			foreach (RomDev.Char character in KickStarter.stateHandler.Characters)
			{
					...
					(charToExclude == null || character != charToExclude))
				{
					...
					grid.AddCharHole (character.transform.position, circleCollider2D.radius);
				}
			}
		}
