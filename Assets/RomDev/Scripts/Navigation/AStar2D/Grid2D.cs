...
namespace RomDev.AStar2D
{
	...
	public class Grid2D
	{
		...
		public void AddCharHole (Vector3 centre, float radius)
		{
			Node centreNode = PositionToNode (centre);
			if (centreNode == null) return;

			Node farLeftNode = PositionToNode (centre - radius * Vector3.right);
			Node farRightNode = PositionToNode (centre + radius * Vector3.right);

			for (int x = farLeftNode.GridX; x <= farRightNode.GridX; x++)
			{
				nodes[x, centreNode.GridY].EvadeCharacter = true;
			}

			Node farTopNode = PositionToNode (centre + radius * Vector3.up);
			Node farBottomNode = PositionToNode (centre - radius * Vector3.up);

			for (int y = farBottomNode.GridY; y <= farTopNode.GridY; y++)
			{
				nodes[centreNode.GridX, y].EvadeCharacter = true;
			}

			// Left fill
			if (centreNode.GridX != farLeftNode.GridX)
			{
				for (int x = farLeftNode.GridX; x <= centreNode.GridX; x++)
				{

					float yLerp = (float) (x - farLeftNode.GridX) / (float) (centreNode.GridX - farLeftNode.GridX);
					int maxY = (int) Mathf.Lerp (farLeftNode.GridY, farTopNode.GridY, yLerp);
					int minY = (int) Mathf.Lerp (farLeftNode.GridY, farBottomNode.GridY, yLerp);
			...
			// Right fill
			if (farRightNode.GridX != centreNode.GridX)
			{
				for (int x = centreNode.GridX; x <= farRightNode.GridX; x++)
				{
					float yLerp = (float) (x - centreNode.GridX) / (float) (farRightNode.GridX - centreNode.GridX);
					int maxY = (int) Mathf.Lerp (farTopNode.GridY, farRightNode.GridY, yLerp);
					int minY = (int) Mathf.Lerp (farBottomNode.GridY, farRightNode.GridY, yLerp);
