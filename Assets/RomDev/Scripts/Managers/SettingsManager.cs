...
namespace RomDev
{
	...
	public class SettingsManager : ScriptableObject
	{
		...
		/** Default save requirement */
		public BasicSaverRequirement basicSaverRequirement;

		public DictionaryBridgeL1<string, SaveRequirement> saveReqBridge = new();
		/** Default save requirement for a specific class */
		public Dictionary<string, SaveRequirement> SaveReqDict
		{
			get
			{
				// if(saveReqDict != null)
				...
				{
					saveReqDict = new();
					saveReqBridge.LinkDictionary(saveReqDict);
					#if UNITY_EDITOR
					#else
					saveReqBridge.Clear();
					#endif
		...
		private Dictionary<string, SaveRequirement> saveReqDict;
		// Save Setting editor vars
		#if UNITY_EDITOR
		private bool showSaveRequirement;
		private Vector2 scrollReqDict;
		public bool clearTargetComp;
		public List<Component> targetSaveComp = new();
		#endif
		#endregion

		#region Prefab Map
		public DictionaryBridgeL1<int, GameObject> prefabMapBridge = new();
		// public DictionaryBridgeL1<GameObject, int> prefabMapObjBridge = new();
		public Dictionary<int, GameObject> PrefabDict
		{
			get
			{
				if(prefabDict == null)
				{
					prefabDict = new();
					prefabMapBridge.LinkDictionary(prefabDict);
					#if UNITY_EDITOR
					#else
					prefabMapBridge.Clear();
					#endif
				}
				return prefabDict;
			}
		}
        public Dictionary<GameObject, int> PrefabDictObj
		{
			get
			{
				if(prefabDictObj == null)
				{
					prefabDictObj = new();
					foreach (KeyValuePair<int, GameObject> prefabDataPair in PrefabDict)
					{
						prefabDictObj.Add(prefabDataPair.Value, prefabDataPair.Key);
					}
				}
				return prefabDictObj;
			}
		}
		private Dictionary<int, GameObject> prefabDict;
        private Dictionary<GameObject, int> prefabDictObj;
		// Prefab Map Setting editor vars
		#if UNITY_EDITOR
		private bool showPrefabMap;
		private bool showPrefabMapDetail;
		private Vector2 scrollPrefabMap;
		public List<GameObject> targetPrefabMap = new();
		#endif
		#endregion

		#region SO Map
		public DictionaryBridgeL1<int, ScriptableObject> soDictBridge = new();
		public Dictionary<int, ScriptableObject> SODict
		{
			get
			{
				if(soDict == null)
				{
					soDict = new();
					soDictBridge.LinkDictionary(soDict);
					#if UNITY_EDITOR
					#else
					soDictBridge.Clear();
					#endif
		...
		private Dictionary<int, ScriptableObject> soDict;

		#if UNITY_EDITOR
		private bool showSOMap;
		private bool showSOMapDetail;
		private Vector2 scrollSOMap;
		public List<ScriptableObject> targetSOMap = new();
		#endif
		#endregion
		...
		/** If True, then this game will be using player system */
		...
		private void ShowSaveGameSettings ()
		{
			...
			if (showSave)
			{
				...
				basicSaverRequirement = (BasicSaverRequirement) CustomGUILayout.ObjectField<BasicSaverRequirement> ("Default Save Req:", basicSaverRequirement, false, "", "Default save requirement for all Basic Object Data Saver");
				if (!string.IsNullOrEmpty (saveFileName))
				{
				...
				#region Save Requirement

				showSaveRequirement = EditorGUILayout.Foldout(showSaveRequirement, "Save Requirement");
				if(showSaveRequirement)
				{
					CustomGUILayout.LabelField("Save Req Dict: ");
					scrollReqDict = GUILayout.BeginScrollView (scrollReqDict, GUILayout.MinHeight(100), GUILayout.MaxHeight(200));
					for (int i= 0; i<saveReqBridge.keys.Count; i++)
					{
						EditorGUILayout.BeginHorizontal ();
						saveReqBridge.values[i] = (SaveRequirement) CustomGUILayout.ObjectField<SaveRequirement> ($"{saveReqBridge.keys[i]}:", saveReqBridge.values[i], false, "", $"Default save requirement for {saveReqBridge.keys[i]}");
						GUILayout.FlexibleSpace ();
						if (GUILayout.Button ("-"))
						{
							saveReqBridge.RemoveData(saveReqBridge.keys[i]);
					...
					CustomGUILayout.LabelField("Target Save Components: ");

					for (int i=0; i<targetSaveComp.Count; i++)
					{
						EditorGUILayout.BeginHorizontal ();
						targetSaveComp[i] = (Component) CustomGUILayout.ObjectField<Component> ("Component to Save: ", targetSaveComp[i], true);
						if (GUILayout.Button ("-"))
						{
							targetSaveComp.RemoveAt (i);
							i=-1;
							EditorGUILayout.EndHorizontal ();
							break;
						}
					...
					if (GUILayout.Button ("Add Target Component"))
					{
						targetSaveComp.Add (null);
					}
					if (GUILayout.Button ("Save Component as Dict"))
					{
						foreach (Component targetSave in targetSaveComp)
						{
							if(targetSave == null) continue;
							string compName = targetSave.GetType().Name;
							if(SaveReqDict.ContainsKey(compName))
							{
								//
								Debug.LogWarning($"Already contains component with type {compName}");
								continue;
							}
							if(targetSave is not ISavable)
							{
								Debug.LogWarning($"component with type {compName} is not Isavable");
								continue;
							}
							saveReqBridge.AddData(compName, null);
						}
						targetSaveComp.Clear();
					}
					#endregion
					//
				}
		...
		private void ShowPrefabMapSettings()
		{
			showPrefabMap = CustomGUILayout.ToggleHeader (showPrefabMap, "Prefab Map");
			if(showPrefabMap)
			{
				CustomGUILayout.BeginVertical();
				showPrefabMapDetail = EditorGUILayout.Foldout (showPrefabMapDetail, "Map Detail");

				if(showPrefabMapDetail)
				{
					// Prefab Map Section
					CustomGUILayout.LabelField("Prefab Map (ID):");
					scrollPrefabMap = GUILayout.BeginScrollView (scrollPrefabMap, GUILayout.MinHeight(40), GUILayout.MaxHeight(400));
					for (int i= 0; i<prefabMapBridge.keys.Count; i++)
					{
						EditorGUILayout.BeginHorizontal ();
						prefabMapBridge.values[i] = (GameObject) CustomGUILayout.ObjectField<GameObject> ($"{prefabMapBridge.keys[i]}:", prefabMapBridge.values[i], false, "", $"Default save requirement for {prefabMapBridge.keys[i]}");
					
						if (GUILayout.Button ("-"))
						{
							prefabMapBridge.RemoveData(prefabMapBridge.keys[i]);
							EditorGUILayout.EndHorizontal ();
							break;
						}
						EditorGUILayout.EndHorizontal ();
				...
				CustomGUILayout.LabelField("Target Prefab Map: ");
				for (int i=0; i<targetPrefabMap.Count; i++)
				{
					EditorGUILayout.BeginHorizontal ();
					targetPrefabMap[i] = (GameObject) CustomGUILayout.ObjectField<GameObject> ("GameObject to Save: ", targetPrefabMap[i], true);
					if (GUILayout.Button ("-"))
					{
						targetPrefabMap.RemoveAt (i);
						i=-1;
						EditorGUILayout.EndHorizontal ();
						break;
					}
				...
				if (GUILayout.Button ("Add Target Prefab"))
				{
					targetPrefabMap.Add (null);
				}
				if (GUILayout.Button ("Save Prefab in Map"))
				{
					foreach (GameObject targetPrefab in targetPrefabMap)
					{
						if(targetPrefab == null) continue;
						if( ! targetPrefab.TryGetComponent<BasicObjectDataSaver>(out BasicObjectDataSaver basicDataSaver))
						{
							Debug.LogWarning($"Prefab with name : {targetPrefab.name} has no BasicObjectdataSaver component !");
							continue;
							// basicDataSaver = targetPrefab.AddComponent<BasicObjectDataSaver>();
						}
						prefabMapBridge.AddData(basicDataSaver.constantID, targetPrefab);
					}
					targetPrefabMap.Clear();
				}

				CustomGUILayout.EndVertical();
			}
		}
		private void ShowSOMapSettings()
		{
			showSOMap = CustomGUILayout.ToggleHeader (showSOMap, "SO Map");
			if(showSOMap)
			{
				CustomGUILayout.BeginVertical();
				showSOMapDetail = EditorGUILayout.Foldout (showSOMapDetail, "SO Map Detail");

				if(showSOMapDetail)
				{
					// SO Map Section
					CustomGUILayout.LabelField("SO Map (ID):");
					scrollSOMap = GUILayout.BeginScrollView (scrollSOMap, GUILayout.MinHeight(40), GUILayout.MaxHeight(400));
					for (int i= 0; i<soDictBridge.keys.Count; i++)
					{
						EditorGUILayout.BeginHorizontal ();
						soDictBridge.values[i] = (ScriptableObject) CustomGUILayout.ObjectField<ScriptableObject> ($"{soDictBridge.keys[i]}:", soDictBridge.values[i], false, "", $"Default save requirement for {soDictBridge.keys[i]}");
					
						if (GUILayout.Button ("-"))
						{
							soDictBridge.RemoveData(soDictBridge.keys[i]);
							EditorGUILayout.EndHorizontal ();
							break;
						}
						EditorGUILayout.EndHorizontal ();
				...
				CustomGUILayout.LabelField("Target Prefab Map: ");
				for (int i=0; i<targetSOMap.Count; i++)
				{
					EditorGUILayout.BeginHorizontal ();
					targetSOMap[i] = (ScriptableObject) CustomGUILayout.ObjectField<ScriptableObject> ("SO to Save: ", targetSOMap[i], false);
					if (GUILayout.Button ("-"))
					{
						targetSOMap.RemoveAt (i);
						i=-1;
						EditorGUILayout.EndHorizontal ();
						break;
					}
				...
				if (GUILayout.Button ("Add Target SO"))
				{
					targetSOMap.Add (null);
				}
				if (GUILayout.Button ("Save SO in Map"))
				{
					foreach (ScriptableObject targetSO in targetSOMap)
					{
						if(targetSO == null) continue;
						if(soDictBridge.ContainsKey(targetSO.GetInstanceID())) continue;
						soDictBridge.AddData(targetSO.GetInstanceID(), targetSO);
					}
					targetSOMap.Clear();
				}
		...
		private void ShowPlayerSettings ()
		{
			...
			{
				CustomGUILayout.BeginVertical ();
				usePlayer = CustomGUILayout.Toggle("Use Player ?", usePlayer,"","If enabled, then this game will using player system");
				if(usePlayer)
				{
		...
		private void ShowInventorySettings ()
		{
			...
			if (showInventory)
			{
				...
				if (CanSelectItems (false))
				{
					...
					{
						if (inventoryInteractions == RomDev.InventoryInteractions.Single || interactionMethod == AC_InteractionMethod.ContextSensitive)
						{
		...
		public int GetIdOfScriptableObject(ScriptableObject _scriptObj)
		{
			foreach (KeyValuePair<int, ScriptableObject> soDictpair in SODict)
			{
				if(soDictpair.Value == _scriptObj)
				{
					return soDictpair.Key;
				}
			}
			return -1;
		}
		...
		private void AssignSaveScriptsInManagers ()
		{
			...
			if (menuManager)
			{
				...
				{
					foreach (RomDev.Menu menu in menuManager.menus)
					{
