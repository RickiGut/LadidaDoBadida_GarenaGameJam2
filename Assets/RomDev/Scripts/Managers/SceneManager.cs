	...
	public class SceneManager : ScriptableObject
	{
		...
		private void SceneSettingsGUI ()
		{
			...
			if (showSettings)
			{
				...
				if(KickStarter.settingsManager.usePlayer)
				{
		...
		public void InitialiseObjects (bool createFolders = true)
		{
			...
			// Assign Player Start
			if (KickStarter.sceneSettings && KickStarter.sceneSettings.defaultPlayerStart == null && KickStarter.settingsManager.usePlayer)
			{
			...
			if (showPrefabs)
			{
				...
				if (prefabTextArray.Length > 0)
				{
					...
					if (GUILayout.Button ("Select", EditorStyles.miniButtonLeft))
					{
						...
						{
							MonoBehaviour[] objects = FindObjectsOfType (Type.GetType ("AC." + activeScenePrefab.componentName)) as MonoBehaviour [];
						...
						{
							MonoBehaviour[] objects = FindObjectsOfType (Type.GetType (activeScenePrefab.componentName)) as MonoBehaviour [];
							if (objects != null && objects.Length > selectedSceneObject && objects[selectedSceneObject].gameObject != null)
							{
								Selection.activeGameObject = objects[selectedSceneObject].gameObject;
							}
		...
		public void GetPrefabsInScene ()
		{
			...
			if (Type.GetType ("RomDev." + activeScenePrefab.componentName) != null)
			{
				objects = FindObjectsOfType (Type.GetType ("RomDev." + activeScenePrefab.componentName)) as MonoBehaviour [];
			...
			{
				Debug.Log(activeScenePrefab.componentName);
				objects = FindObjectsOfType (Type.GetType (activeScenePrefab.componentName)) as MonoBehaviour [];
				foreach (MonoBehaviour _object in objects)
				{
					if (UnityVersionHandler.ObjectIsInActiveScene (_object.gameObject))
					{
		...
		public void AddPrefab (SceneManagerPrefabData newData)
		{
			...
			KickStarter.sceneManager.RegneratePrefabCache ();
