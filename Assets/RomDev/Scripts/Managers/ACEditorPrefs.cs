...
namespace RomDev
{
	...
	public class ACEditorPrefs : ScriptableObject
	{
		...
		private const string DefaultInstallPath = "Assets/RomDev";
		...
		[SerializeField] protected string installPath = "Assets/RomDev";
	...
	static class ACEditorPrefsIMGUIRegister
	{
		...
		{
			var provider = new SettingsProvider ("Project/RomDev", SettingsScope.Project)
			{
				label = "RomDev",
				...
				guiHandler = (searchContext) =>
				{
					...
					{
						EditorGUILayout.HelpBox ("Cannot create AC editor prefs - does the folder '/Assets/RomDev/Editor' exist?", MessageType.Warning);
					}
				},
			};
