...
namespace RomDev
{
	...
	public struct ConstantIDManager
	{
		...
		public bool IsContains(ConstantID _constantId)
		{
			foreach (ConstantID constID in constantIDs)
			{
				if(_constantId == constID)
				{
					return true;
				}
			}
		...
		public bool OverwriteBasicSaverID(ConstantID _constantID, int newID)
        {
            ConstantID foundConstant = GetConstantID(_constantID.constantID);
            if( foundConstant == null)
            {
                return false;
            }
            foundConstant.constantID = newID;
			return true;
        }
