...
namespace RomDev
{
	...
	public abstract class Template : ScriptableObject
	{
		...
		[SerializeField] private SceneAsset[] exampleScenes = new SceneAsset[0];
		...
		protected enum ExistingMenuBehaviour { Ignore, Rename, Delete}
		protected bool CopyMenus (string installPath, MenuManager source, MenuManager destination, ExistingMenuBehaviour existingMenuBehaviour = ExistingMenuBehaviour.Rename)
		{
						...
						for (int i = 0; i < destination.menus.Count; i++)
						{
							if (destination.menus[i].title == defaultMenu.title)
							{
								...
								else if (existingMenuBehaviour == ExistingMenuBehaviour.Delete)
								{
									...
									KickStarter.menuManager.menus.RemoveAt (i);
		...
		protected T CopyAsset<T> (string installPath, T original, string extension) where T : UnityEngine.Object
		{
		...
		public virtual Texture2D PreviewTexture { get { return null; } }
