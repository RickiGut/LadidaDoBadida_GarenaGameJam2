	...
	public class CharacterWizardWindow : EditorWindow
	{
		...
		private void OnGUI ()
		{
			...
			if (pageNumber < numPages - 1)
			{
				if (pageNumber == 1)
				{
					if (!IsFirstPerson ())
					{
						if (baseObject == null || UnityVersionHandler.IsPrefabFile (baseObject) || baseObject.GetComponent <RomDev.Char>() || !baseObject.activeInHierarchy)
						{
							GUI.enabled = false;
						}
					}
		...
		private void Finish ()
		{
			...
			if (is2D)
			{
				...
				charScript.turn2DCharactersIn3DSpace = false;
			}
			else
			{
			...
			if (animationEngine == RomDev.AnimationEngine.Custom)
			{
		...
		private void ShowPage ()
		{
			...
			else if (pageNumber == 1)
			{
				...
				{
					if (baseObject.GetComponent <RomDev.Char>())
					{
