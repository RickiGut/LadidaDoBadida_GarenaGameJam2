	...
	public class ActionsManager : ScriptableObject
	{
		...
		private void ShowEditingGUI ()
		{
			...
			if (showEditing)
			{
				...
				displayActionsInInspector = CustomGUILayout.Toggle ("List Actions in Inspector?", displayActionsInInspector, "RomDev.KickStarter.actionsManager.displayActionsInInspector", "If True, then Actions can be displayed in an ActionList's Inspector window");
				displayActionsInEditor = (DisplayActionsInEditor) CustomGUILayout.EnumPopup ("Actions in Editor are:", displayActionsInEditor, "RomDev.KickStarter.actionsManager.displayActionsInEditor", "How Actions are arranged in the ActionList Editor window");
				actionListEditorScrollWheel = (ActionListEditorScrollWheel) CustomGUILayout.EnumPopup ("Using scroll-wheel:", actionListEditorScrollWheel, "RomDev.KickStarter.actionsManager.actionListEditorScrollWheel", "The effect the mouse scrollwheel has inside the ActionList Editor window");
				...
				autoPanNearWindowEdge = CustomGUILayout.Toggle ("Auto-panning in Editor?", autoPanNearWindowEdge, "RomDev.KickStarter.actionListManager.autoPanNearWindowEdge", "If True, the ActionList Editor will pan automatically when dragging the cursor near the window's edge");
				panSpeed = CustomGUILayout.FloatField ((actionListEditorScrollWheel == ActionListEditorScrollWheel.PansWindow) ? "Panning speed:" : "Zoom speed:", panSpeed, "RomDev.KickStarter.actionsManager.panSpeed", "The speed factor for panning/zooming");
				invertPanning = CustomGUILayout.Toggle ("Invert panning in Editor?", invertPanning, "RomDev.KickStarter.actionsManager.invertPanning", "If True, then panning is inverted in the ActionList Editor window (useful for Macbooks)");
				focusOnPastedActions = CustomGUILayout.Toggle ("Focus on pasted Actions?", focusOnPastedActions, "RomDev.KickStarter.actionListManager.focusOnPastedActions", "If True, then the ActionList Editor window will focus on newly - pasted Actions");
				allowMultipleActionListWindows = CustomGUILayout.Toggle ("Allow multiple Editors?", allowMultipleActionListWindows, "RomDev.KickStarter.actionsManager.allowMultipleActionListWindows", "If True, then multiple ActionList Editor windows can be opened at once");
			...
			if (actionList != null)
			{
				foreach (Action action in actionList)
				{
					...
					if ((action.Category == actionType.category && action.Title == actionType.title) ||
					    ...
					    (action.GetType ().ToString () == "RomDev." + actionType.fileName))
					{
		...
		public bool DoesActionExist (string _name)
		{
			...
			{
				if (_name == actionType.fileName || _name == ("RomDev." + actionType.fileName))
				{
					return true;
				}
			}
		...
		public int GetActionTypeIndex (Action _action)
		{
			if (_action != null)
			{
				...
				className = className.Replace ("RomDev.", "");
				foreach (ActionType actionType in AllActions)
				{
		...
		{
			string fileName = _action.GetType ().ToString ().Replace ("RomDev.", "");
