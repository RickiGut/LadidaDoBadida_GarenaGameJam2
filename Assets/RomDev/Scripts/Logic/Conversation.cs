...
namespace RomDev
{
	...
	public class Conversation : MonoBehaviour, ITranslatable, iActionListAssetReferencer, IItemReferencer, IVariableReferencer
	{
		...
		public RomDev.InteractionSource interactionSource;
		...
		public void RunOptionWithID (int ID, bool force = false)
		{
			...
			if (!gameObject.activeInHierarchy || interactionSource == RomDev.InteractionSource.CustomScript)
			{
				RunOption (buttonDialog);
			}
			else
		...
		protected void RunOption (ButtonDialog _option)
		{
			...
			if (interactionSource != RomDev.InteractionSource.CustomScript)
			{
			...
			if (interactionSource == RomDev.InteractionSource.AssetFile && _option.assetFile)
			{
				AdvGame.RunActionListAsset (_option.assetFile, endConversation);
			}
			else if (interactionSource == RomDev.InteractionSource.CustomScript)
			{
			...
			else if (interactionSource == RomDev.InteractionSource.InScene && _option.dialogueOption)
			{
		...
		{
			if (interactionSource == RomDev.InteractionSource.AssetFile)
			{
				foreach (ButtonDialog buttonDialog in options)
				{
