...
namespace RomDev
{
	...
	public class ActionMenuState : Action, ITranslatable, IMenuReferencer
	{
		...
		public override float Run ()
		{
			if (!isRunning)
			{
				RomDev.Menu _menu = PlayerMenus.GetMenuWithName (runtimeMenuToChange);
		...
		protected void RunInstant (RomDev.Menu _menu)
		{
			switch (changeType)
			{
				...
				case MenuChangeType.TurnOnMenu:
					{
						...
						{
							menuToChange = TextField ("Menu to turn on:", menuToChange);
						}
				...
				case MenuChangeType.TurnOffMenu:
					{
						...
						if (menuToChangeParameterID < 0)
						{
							menuToChange = TextField ("Menu to turn off:", menuToChange);
						}
						doFade = EditorGUILayout.Toggle ("Transition?", doFade);
					}
					break;
				...
				case MenuChangeType.HideMenuElement:
					{
						...
						if (menuToChangeParameterID < 0)
						{
							menuToChange = TextField ("Menu containing element:", menuToChange);
						}
						...
						{
							elementToChange = TextField ("Element to hide:", elementToChange);
						}
					}
					break;
