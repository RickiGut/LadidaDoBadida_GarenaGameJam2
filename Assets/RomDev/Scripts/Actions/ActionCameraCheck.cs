...
namespace RomDev
{
	...
	public class ActionCameraCheck : ActionCheck
	{
		...
		{
			parameterID = Action.ChooseParameterGUI ("Camera to check:", parameters, parameterID, ParameterType.GameObject);
			if (parameterID >= 0)
			{
				constantID = 0;
				cameraToCheck = null;
			...
			{
				cameraToCheck = (_Camera) EditorGUILayout.ObjectField ("Camera to check:", cameraToCheck, typeof (_Camera), true);
				
				constantID = FieldToID (cameraToCheck, constantID);
				cameraToCheck = IDToField (cameraToCheck, constantID, false);
			}
