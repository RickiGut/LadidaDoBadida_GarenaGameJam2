...
namespace RomDev
{
	...
	public class ActionMenuCheck : ActionCheck, IMenuReferencer
	{
		...
		{
			RomDev.Menu _menu = PlayerMenus.GetMenuWithName (_menuToCheck);
			if (_menu != null)
			{
		...
		public override void ShowGUI (List<ActionParameter> parameters)
		{
			...
			if (checkType == MenuCheckType.MenuIsVisible || checkType == MenuCheckType.MenuIsLocked)
			{
				...
				{
					menuToCheck = TextField ("Menu to check:", menuToCheck);
				}
			}
			else if (checkType == MenuCheckType.ElementIsVisible)
			{
				...
				if (menuToCheckParameterID < 0)
				{
					menuToCheck = TextField ("Menu containing element:", menuToCheck);
				}
				...
				{
					elementToCheck = TextField ("Element to check:", elementToCheck);
				}
			}
		}
