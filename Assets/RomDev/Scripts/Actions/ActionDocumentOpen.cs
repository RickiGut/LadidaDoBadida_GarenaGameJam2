	...
	public class ActionDocumentOpen : Action, IDocumentReferencerAction
	{
		...
		public override void ShowGUI (List<ActionParameter> parameters)
		{
			...
			if (setElement)
			{
				...
				elementParameterID = Action.ChooseParameterGUI ("Journal name:", parameters, elementParameterID, new ParameterType[2] { ParameterType.String, ParameterType.PopUp });
				if (elementParameterID < 0)
				{
					documentElementName = TextField ("Journal name:", documentElementName);
				}
			}
			else
			{
