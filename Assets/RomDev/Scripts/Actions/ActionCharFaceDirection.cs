...
namespace RomDev
{
	...
	public class ActionCharFaceDirection : Action
	{
		...
		public override void ShowGUI (List<ActionParameter> parameters)
		{
			...
			if (directionParameterID < 0)
			{
				direction = (Direction) EditorGUILayout.EnumPopup ("Direction to face:", direction);
			}

			if (directionParameterID < 0 && direction == Direction.SetDirection)
			{
				vectorParameterID = Action.ChooseParameterGUI ("Direction:", parameters, vectorParameterID, ParameterType.Vector3);
				if (vectorParameterID < 0)
					vector = EditorGUILayout.Vector3Field ("Direction:", vector);
			}
			else if (directionParameterID < 0 && direction == Direction.SetPosition)
			{
				vectorParameterID = Action.ChooseParameterGUI ("Position:", parameters, vectorParameterID, ParameterType.Vector3);
				if (vectorParameterID < 0)
					vector = EditorGUILayout.Vector3Field ("Position:", vector);
			}
			else
			{
		 ...
		 */
		public static ActionCharFaceDirection CreateNew (RomDev.Char characterToTurn, CharDirection directionToFace, RelativeTo relativeTo = RelativeTo.Camera, bool isInstant = false, bool waitUntilFinish = false)
		{
