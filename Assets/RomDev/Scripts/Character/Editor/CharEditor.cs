...
namespace RomDev
{
	...
	[CustomEditor (typeof (RomDev.Char))]
...

	{
		...
		protected void SharedGUIOne (RomDev.Char _target)
		{
			...
			CustomGUILayout.Header ("Movement");
			CustomGUILayout.BeginVertical ();
		...
		protected void SharedGUITwo (RomDev.Char _target)
		{
			CustomGUILayout.Header ("Physics");
			CustomGUILayout.BeginVertical ();
			...
			CustomGUILayout.Header ("Dialogue");
			CustomGUILayout.BeginVertical ();
		...
		private string GetTokensList (RomDev.Char _target)
		{
			string result = "\n  [wait]\n  [wait:X]\n  [hold]\n  [continue]\n  [speaker]\n  [line:ID]\n  [token:ID]";
			if (_target.useExpressions)
			{
		...
		private void ExpressionSideMenu (RomDev.Char _target, int i)
		{
		...
		{
			RomDev.Char t = (RomDev.Char) target;
			ModifyAction (t, expressionToAffect, obj.ToString ());
			EditorUtility.SetDirty (t);
		}
		
		
		private void ModifyAction (RomDev.Char _target, Expression _expression, string callback)
		{
			int i = -1;
