...
namespace RomDev
{
	...
	public class GVar : ITranslatable
	{
		...
		/** If True, then presets will not be applied to this variable */
		...
		public GVar (GVar assetVar)
		{
			...
			ignorePresets = assetVar.ignorePresets;
		...
		public void CopyFromVariable (GVar oldVar, VariableLocation oldLocation)
		{
			...
			switch (type)
			{
				...
				case VariableType.String:
					{
						string oldValue = oldVar.GetValue (Options.GetLanguage ());
						textVal = oldValue;
		...
		public void ShowGUI (VariableLocation location, bool canEdit, List<VarPreset> _varPresets = null, string apiPrefix = "", Variables _variables = null)
		{
			...
			if (_varPresets != null && _varPresets.Count > 0)
			{
				EditorGUILayout.Space ();
				ignorePresets = CustomGUILayout.Toggle ("Ignore presets?", ignorePresets);
				...
				if (!ignorePresets)
				{
					foreach (VarPreset _varPreset in _varPresets)
					{
						...
						string label = "'" +
						(!string.IsNullOrEmpty (_varPreset.label) ? _varPreset.label : ("Preset #" + _varPreset.ID.ToString ())) + "' value:";
