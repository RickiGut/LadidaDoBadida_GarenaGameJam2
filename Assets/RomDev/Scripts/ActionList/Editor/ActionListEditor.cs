...
namespace RomDev
{
	...
	public class ActionListEditor : Editor
	{
		...
		private RomDev.Action actionToAffect = null;
		...
		protected void DrawSharedElements (ActionList _target)
		{
			...
			else
			{
				...
				if (GUILayout.Button ("Expand all", EditorStyles.miniButtonLeft))
				{
					...
					foreach (RomDev.Action action in _target.actions)
					{
		...
		public static int ShowTypePopup (RomDev.Action action, int typeIndex)
		{
		...
		public static RomDev.Action RebuildAction (RomDev.Action existingAction, int typeIndex, ActionList _target, int insertIndex = -1, ActionEnd _end = null)
		{
		...
		private void ActionSideMenu (int i)
		{
			...
			ActionMenuItem[] menuItems = actionToAffect.GetMenuItems ();
			if (menuItems != null && menuItems.Length > 0)
			{
				menu.AddSeparator (string.Empty);
				for (int c = 0; c < menuItems.Length; c++)
				{
					menu.AddItem (new GUIContent (menuItems[c].Label), false, Callback, "Custom" + c.ToString ());
				}
			}
		...
		public static void ModifyAction (ActionList _target, RomDev.Action _action, string callback)
		{
			int i = -1;
			...
			switch (callback)
			{
				...
				default:
					if (callback.StartsWith ("Custom"))
					{
						string postCustom = callback.Substring ("Custom".Length);
						int customIndex = 0;
						if (int.TryParse (postCustom, out customIndex))
						{
							foreach (Action action in _target.actions)
							{
								if (action != null && action.isMarked)
								{
									...
									{
										action.GetMenuItems ()[customIndex].Callback.Invoke (_target.actions);
										break;
									}
								}
							}
		...
		public static void DeleteAction (RomDev.Action action, ActionList _target)
		{
			if (action != null) 
			{
		...
		public static Action AddAction (string className, int i, ActionList _target)
		{
			...
			foreach (RomDev.Action _action in _target.actions)
			{
		...
		public static Action AddAction (RomDev.Action newAction, int i, ActionList _target)
		{
			if (i < 0)
			{
		...
		public static void PushNodes (List<RomDev.Action> list, float xPoint, int count)
		{
			foreach (RomDev.Action action in list)
			{
